var Bt=Object.defineProperty;var Mt=(e,t)=>{for(var i in t)Bt(e,i,{get:t[i],enumerable:!0})};async function N(e){if(!(!e.dynamic&&e.create instanceof Function)){if(e.create instanceof Function){e.create();return}if(typeof e.target=="string"&&(e.target=document.getElementById(e.target)),!(e.target instanceof HTMLElement)){console.warn("Dataviews require a HTMLHtmlElement target");return}return Ht(e)instanceof Error?e.err:(typeof e.label=="string"&&(e.chkbox=mapp.ui.elements.chkbox({checked:!!e.display,data_id:e.key,disabled:e.disabled,label:e.label,onchange:t=>{e.display=t,e.display?e.show():e.hide()}})),e.update??=Rt,e.show??=Vt,e.hide??=Pt,mapp.ui.utils.dataview.Toolbar(e),mapp.ui.utils.dataview.mapChange(e),e)}}async function Vt(){this.display=!0,this.create===void 0?(this.create=()=>{mapp.ui.utils[this.dataview].create(this)},this.create(),typeof this.update=="function"&&this.update()):this.dynamic?(typeof this.create=="function"&&await this.create(),typeof this.update=="function"&&this.update()):this.reload&&typeof this.update=="function"&&this.update(),this.btnRow?.style.setProperty("display","flex"),this.target.style.display="block"}function Pt(){this.display=!1,this.target.style.display="none",this.btnRow?.style.setProperty("display","none")}function Ht(e){if(e.key??=e.query||e.title||e.label,e.dataview||(e.chart&&(e.dataview="chartjs"),typeof e.columns<"u"&&(console.warn("Table dataviews should be configured inside a tables object"),e.table={columns:e.columns},e.dataview="tabulator"),e.table&&(e.dataview="tabulator")),!Object.hasOwn(mapp.ui.utils,e.dataview))return e.err=new Error(`mapp.ui.utils.${e.dataview} doesnt exist`),console.error(e.err),e.update=()=>console.warn(`Unable to update ${e.key} dataview.`),e.err;if(typeof mapp.ui.utils[e.dataview].create!="function")return e.err=new Error(`mapp.ui.utils.${e.dataview}.create() method doesn't exist`),console.error(e.err),e.err}async function Rt(){if(this.create===void 0&&(this.create=function(){mapp.ui.utils[this.dataview].create(this)},this.create()),!this.query)return;let e=mapp.utils.queryParams(this),t=mapp.utils.paramString(e);this.host??=this.layer?.mapview?.host;let i=await mapp.utils.xhr(`${this.host||mapp.host}/api/query?${t}`);if(!(i instanceof Error)){if(typeof this.responseFunction=="function"){this.responseFunction(i);return}typeof this.setData=="function"&&this.setData(i)}}function F(e){typeof e=="string"&&(e={text:e}),e.title??=mapp.dictionary.information,e.icon??="info",e.data_id??="alert",e.class??="alert-confirm box-shadow no-select";let t=e.icon===""?"":mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;return e.header=mapp.utils.html`${t}${e.title}`,e.innerContent??=mapp.utils.html`<p>${e.text}</p>`,e.content=mapp.utils.html`
    ...
    <button 
        ...
        ${e.panel&&mapp.utils.html`
            <div class="panel">${e.panel}`}`;function I(e){return e.data_id??="card",mapp.utils.html.node`
  <div 
    data-id=${e.data_id}
    class="drawer">
    ...
    ${e.content}`}function z(e){return e.data_id??="chkbox",e.name??="mapp-ui-chkbox-element",mapp.utils.html.node`<label 
    data-id=${e.data_id}
    class="checkbox">
    <input
      name=${e.name}
    ...
    <span>${e.label}`}function D(e){typeof e=="string"&&(e={text:e}),e.title??=mapp.dictionary.confirm,e.data_id??="confirm",e.icon??="warning";let t=e.icon===""?"":mapp.utils.html`<span class="material-symbols-outlined notranslate">${e.icon}</span>`;e.header=mapp.utils.html`${t}${e.title}`,delete e.minimizeBtn,delete e.closeBtn;let i=mapp.utils.html.node`<button 
      class="action outlined bold">
  ...
  <li
    ...
    class=${e.class}>
    ${e.headerHtml}
    <div class="content">${e.content}`,e.modal?(document.body.append(e.node),e.node.showModal()):(e.target.appendChild(e.node),e.node.show(),e.right&&(e.left=e.target.offsetWidth-e.node.offsetWidth-Number.parseInt(e.right)),e.node.style.top=`${e.top||0}`,e.node.style.left=`${e.left||0}`,e.node.addEventListener("mousedown",i),e.node.addEventListener("touchstart",i),new ResizeObserver(o).observe(e.target));function i(a){if(e.headerDrag&&!a.target.closest("header")||a.which===3)return;e.node.style.cursor="grabbing";let s={mousedown:{end:"mouseup",move:"mousemove"},touchstart:{end:"touchend",move:"touchmove"}},{move:d,end:c}=s[a.type];e.target.addEventListener(d,n),window.addEventListener(c,l)}function o(){let{offsetWidth:a,offsetHeight:s}=e.target,{offsetWidth:d,offsetHeight:c}=e.node,u=a-d<0?0:a-d,b=s-c<0?0:s-c;e.node.style.left=`${Math.min(Math.max(e.node.offsetLeft,0),u)}px`,e.node.style.top=`${Math.min(Math.max(e.node.offsetTop,0),b)}px`}function l(){delete e.x,delete e.y,e.node.style.cursor=e.headerDrag?"auto":"grab",e.target.removeEventListener("mousemove",n),e.target.removeEventListener("touchmove",n),e.target.removeEventListener("mouseup",l),e.target.removeEventListener("touchend",l)}function n(a){e.x??=a.x;let s=a.x-e.x;e.x=a.x,e.y??=a.y;let d=a.y-e.y;if(e.y=a.y,e.contained&&!e.modal){Zt(e,s,d);return}if(e.containedCentre&&!e.modal){Yt(e,s,d);return}e.node.style.left=`${e.node.offsetLeft+s}px`,e.node.style.top=`${e.node.offsetTop+d}px`}return e.dialog=e,e}function Zt(e,t,i){e.node.offsetLeft+t<0?e.node.style.left=0:t<0?e.node.style.left=`${e.node.offsetLeft+t}px`:e.target.offsetWidth-e.node.offsetWidth-e.node.offsetLeft>0&&(e.node.style.left=`${e.node.offsetLeft+t}px`),e.node.offsetTop+i<0?e.node.style.top=0:i<0?e.node.style.top=`${e.node.offsetTop+i}px`:e.target.offsetHeight-e.node.offsetHeight-e.node.offsetTop>0&&(e.node.style.top=`${e.node.offsetTop+i}px`)}function Yt(e,t,i){e.node.offsetLeft+Number.parseInt(e.node.offsetWidth/2)+t<0||(t<0?e.node.style.left=`${e.node.offsetLeft+t}px`:e.target.offsetWidth-Number.parseInt(e.node.offsetWidth/2)-e.node.offsetLeft>0&&(e.node.style.left=`${e.node.offsetLeft+t}px`),!(e.node.offsetTop+Number.parseInt(e.node.offsetHeight/2)+i<0)&&(i<0?e.node.style.top=`${e.node.offsetTop+i}px`:e.target.offsetHeight-Number.parseInt(e.node.offsetHeight/2)-e.node.offsetTop>0&&(e.node.style.top=`${e.node.offsetTop+i}px`)))}function Xe(){this.node&&this.target instanceof HTMLElement&&this.target.append(this.node)}function Qe(e){if(e.data_id??="drawer",e.dialog)return M(e);if(e.drawer===!1)return e.drawer=mapp.utils.html.node`<div
  ...
  <button
    ...
    <h3>${mapp.dictionary.circle_config}</h3>`,content:o,popout:i.popout,dialog:i.dialog}),i.drawer)}function Ke(e,t){let i=e.unitsConfig[t];e.units=i.option,e.radius??=i.min,e.radius=e.radius>i.max?i.max:e.radius,mapp.utils.render(e.rangeSlider,mapp.ui.elements.slider({callback:o=>{e.radius=Number.parseFloat(o)},max:i.max,min:i.min,val:e.radius}))}function ai(e){return e.draw.locator={label:mapp.dictionary.draw_position,layer:e,type:"Point",...e.draw.locator},e.draw.locator.btn=mapp.utils.html.node`
    <button
      class="action wide"
  ...
  <div
    class="val"
    style="${e.css_val}">${t}`;return!e.edit&&e.type==="html"&&(i.innerHTML=e.value||""),i}function Fe(e){let t,i=e.value?.toString().replace(".",":");return i=i&&i.length<3&&`${i}:00`||i,e.edit?t=mapp.utils.html.node`
      <input
        type="time"
        value=${i}
