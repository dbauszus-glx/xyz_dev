...
describe('workspace: roles_object_workspace', () => {
  ...
  it('roles objects should not create dot notation roles', async () => {
    ...
    const scopesWorkspace = await checkWorkspaceCache();
    scopesWorkspace.scopes.add(['added-after-scope-generation']);
    ...
    await getKeyMethod(cachedReq, cachedRes);

    expect(cachedRes._getData()).toEqual(expectedRoles);
    expect(await checkWorkspaceCache()).toBe(scopesWorkspace);
  });
});
...
describe('workspace: cached scopes', () => {
  beforeAll(async () => {
    ...
    workspace.cachedScopes = ['precomputed.scope'];
  });
  ...
  it('returns scopes generated by workspace:cache without composing locales', async () => {
    ...
    expect(res._getData()).toEqual(['precomputed.scope']);
