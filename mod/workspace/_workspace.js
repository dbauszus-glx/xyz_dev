...
@description
The scopes method returns the cachedScopes array from the workspace.

The cachedScopes are generated by the workspace:cache utility. Scopes are not generated at runtime.

@param {req} req HTTP request.
@param {res} res HTTP response.

@property {Object} req.params HTTP request parameter.
@property {Boolean} [params.tree] Whether the scopes should be returned as tree structure.
...
async function scopes(req, res) {
  ...
  if (!Array.isArray(cachedWorkspace.cachedScopes)) {
    res
      .status(404)
      .setHeader('Content-Type', 'text/plain')
      .send('The workspace has no cachedScopes.');
    return;
  }

  if (req.params.tree) {
    scopesArrayToTree(res, cachedWorkspace.cachedScopes);
    return;
  }
